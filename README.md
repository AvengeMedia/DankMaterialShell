...
Adds a chat system to DMS, built so that messaging services arrive as plugins rather than as
code baked into the shell. WhatsApp, Signal, a mail account and so on can each be added later
without touching anything here.

### How a chat plugin works

A chat plugin ships a small program called a **bridge** instead of QML. The bridge does one job:
it translates its service's protocol into JSON lines on stdout, and reads commands as JSON lines
on stdin. It can be written in any language.

Everything that is actually hard is done once, in the Go backend, and shared by every provider:
storing messages, counting unread, paginating history, caching attachments, raising desktop
notifications, and searching. A bridge never opens the database, never writes to the media cache
and never decides what to notify about, which is what keeps a provider to a few hundred lines.

The full contract is in `docs/CHAT-PLUGINS.md`, and `quickshell/PLUGINS/EchoChatExample/` is a
complete working bridge to copy.

### What was added

**Backend.** A shared SQLite message store covering every provider at once, so the conversation
list and the Super+Tab rotation can span services in a single query. Messages page backwards by
keyset rather than offset, so scrolling a long history stays fast; delivery status only ever moves
forward, so a redelivered old message cannot un-read something already seen. Attachments are cached
on disk with a size cap and evicted least-recently-used. Notifications apply a suppression ladder —
never for your own messages, the conversation on screen, muted or archived chats, or the backfill
that arrives on first sign-in.

Bridges are supervised child processes: started when a provider is enabled, restarted with backoff
if they die, and stopped cleanly when disabled.

**Settings → Chats.** A new section with notification and storage preferences, plus a row per
installed provider showing its connection state and unread count. Each provider's own settings
component loads inline there, and changes are pushed straight down to its running bridge — a bridge
never reads the shell's config files, which is what lets one be developed and tested entirely
outside DMS.

**Chat window.** A conversation list and message view that know nothing about any particular
service. Each provider declares what it supports, and the UI hides what it does not — no attach
button without attachment support, no reply affordance without replies. Reachable via
`dms ipc call chats toggle` or a keybind, including a cycle action that walks unread conversations
across every provider.

**Tooling.** `dms chat providers`, `dms chat status <id>` and `dms chat tail <id>` for debugging a
bridge — the last streams its protocol traffic live, which matters because a bridge runs as a child
of the daemon and is otherwise invisible to its own author. A chat plugin guide and a Python bridge
template were added to the plugin-dev skill.

### Note

This adds `modernc.org/sqlite`, the first SQL driver in `core`. It is pure Go with no cgo. It was
chosen over the existing bbolt because message search and derived counts are queries in SQL but
hand-maintained invariants in a key-value store.

No messaging provider is implemented here — only the framework and the echo bridge used to test it.
