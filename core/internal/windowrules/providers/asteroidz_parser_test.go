...
func TestParseAsteroidzWindowRuleLine(t *testing.T) {
	fields := parseAsteroidzWindowRuleLine("match app-id=firefox title=Gmail; open-floating; tags 2; monitor HDMI-A-1")
...
func TestConvertAsteroidzRulesToWindowRules(t *testing.T) {
	mangoRules := []AsteroidzWindowRule{
		{Source: "config.conf", Fields: parseAsteroidzWindowRuleLine("match app-id=discord; tags 9; open-floating; no-blur")},
	}
	rules := ConvertAsteroidzRulesToWindowRules(mangoRules)
	if len(rules) != 1 {
...
func TestAsteroidzSetAndLoadRoundTrip(t *testing.T) {
	tmpDir := t.TempDir()
	provider := NewAsteroidzWritableProvider(tmpDir)
