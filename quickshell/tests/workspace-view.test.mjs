...
methods(service, serviceSource, ["activateWorkspace", "commandPayload"]);
service.command = (action, fields) => assert.throws(() => service.commandPayload(action, fields), /stale session/);
service.activateWorkspace(first);
assert.throws(() => service.commandPayload(clicked.at(-1).action, clicked.at(-1).fields), /stale session/);
