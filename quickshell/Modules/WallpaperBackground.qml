        ...
        Item {
            ...
            Connections {
                ...
                function onIsShellLockedChanged() {
                    if (!IdleService.isShellLocked) {
                        root._renderSettling = true;
                        renderSettleTimer.restart();
                        // Unilateral re-attach on every unlock for parallax-
                        // active surfaces — catches both detected rebinds and
                        // silent ones the lifecycle signals miss. Non-parallax
                        // wallpapers don't have the wedge risk so leave them be.
                        if (root.effectiveScrolling) root._reattachSurface();
                    }
                }
            }
            ...
            Component.onCompleted: {
                if (typeof wallpaperWindow.updatesEnabled !== "undefined")
                    wallpaperWindow.updatesEnabled = Qt.binding(() => !root.source || root.effectActive || root._renderSettling || root.overviewBlurActive || root._overviewBlurSettling || root.pendingWallpaper !== "" || root._deferredSource !== "" || frameAnim.running || currentWallpaper.status === Image.Loading || nextWallpaper.status === Image.Loading);
