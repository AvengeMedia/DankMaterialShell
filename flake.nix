{
  ...
  outputs =
    ...
    let
      ...
      # Extracted so both `packages.<system>.dms-shell` (for direct consumers:
      # `nix build .#`) and `buildDmsPkgs pkgs` (for NixOS/home-manager modules)
      # build the derivation with a caller-supplied `pkgs`. Previously the
      # module path reused the pre-built `self.packages.<system>.default`,
      # which was instantiated from `nixpkgs.legacyPackages` — an unoverlayed
      # pkgs instance. That made it impossible for downstream flakes to reach
      # through their own overlays to the build (e.g. to swap kdePackages.sonnet
      # or trim perl out of aspell). With this refactor, the module's `pkgs`
      # arg (which carries the user's overlays) is used for the actual build.
      mkDmsShell =
        pkgs:
        let
          mkDate =
